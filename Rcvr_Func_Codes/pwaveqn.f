...
c							CJA 880722
c
c    Minor conversions, June 2007, TJO, for fortran-95 conversion (added MAXPTS)
c
c  *************************************************************************
c
      integer, parameter :: MAXPTS = 4100
      dimension d2(MAXPTS),caz(3)
      character eqfile*256,outfil*256,comp(3,2)*6,outc(2)*5,knm(2)*8
      complex data(MAXPTS,3),zero1
...
c
    ...
    2 call asktxt('Specify quake file: ',eqfile)
      ...
      if(rel) isyntp=1
      do 1 i=1,3
         call zeerow(data(1,i),1,2*MAXPTS)
         eqfile(1:iblank + 6)=eqfile(1:iblank)//comp(i,isyntp)
c
c set MAXPTS for file size test on read
c
         npts=MAXPTS
         call sacio(eqfile,data(1,i),npts,dt,+1)
         caz(i)=az
    1 continue
      if(rel) call rotseis(data,2*MAXPTS,3,baz,caz,npts)
c
c    *******************************************************************
c
...
c
 ...
 111  continue
   ...
   10 yes=yesno('Try another (y or n)? ')
      ...
      subroutine windoe(b,npts,dt,ndat)
      integer, parameter :: MAXPTS = 4100
      dimension data(2*MAXPTS,3)
      common /win/ data
      data pi/3.1415926/
      bb=pi/b
      nend=ifix((b/dt)+.5) +1
